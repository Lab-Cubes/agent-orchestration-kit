...
- **NPT terminology renamed to Cognon (CGN).** Runtime docs now use
  Cognon/CGN naming for budget and cost accounting, matching the
  NPS-Release `spec/token-budget.md` v0.3 Cognon Budget Specification.
...
- Cognon (CGN) approximation formula (§8) updated to be runtime-agnostic.
...
- CSV cost logging (13-col schema: timestamp, task_id, agent_id, model,
  category, priority, budget_cgn, cost_cgn, cost_usd_derived, turns,
  duration_s, denials, status)
- Discord plugin (`plugins/discord`)
- Cost-monitor plugin (`plugins/cost-monitor`)
- `bin/benchmark` — live Cognon (CGN) comparison (naive prompt vs NOP dispatch)
- Tested across 40+ real tasks before release

### Known approximations

- Cognon (CGN) cost is v0.1.0 approximation (`input + output + cache_read` tokens);
  full four-channel normalization targets v0.2.0.
