...
@test "#207 no-lifecycle: unclaimed intent fails even when worker wrote result" {
...
HOOK
    ...
    run_spawner setup coder-01 coder
    run run_spawner dispatch coder-01 "orphan result lifecycle test" --category code --time-limit 60
...
d = json.load(open(sys.argv[1]))
assert d["payload"]["status"] == "completed", d["payload"]["status"]
assert d["value"] == "Mock worker wrote result without claiming.", d["value"]
print("ok")
PYEOF
    [ "$status" -eq 0 ]
