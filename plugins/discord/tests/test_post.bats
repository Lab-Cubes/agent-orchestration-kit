...
@test "_post.sh includes User-Agent header with valid config" {
    cp "$FIXTURES/config.json" "$PLUGIN_TMPDIR/config.json"
...
@test "_post.sh reads token from openclaw.json when token_from_openclaw is set and file exists" {
    ...
    python3 - "$FIXTURES/config.json" "$FIXTURES/openclaw.json" "$PLUGIN_TMPDIR/config.json" << 'PYEOF'
import json, sys
...
@test "_post.sh exits 0 (no error) when channel_id is empty" {
    python3 - "$FIXTURES/config.json" "$PLUGIN_TMPDIR/config.json" << 'PYEOF'
import json, sys
