#!/usr/bin/env bash
# Discord plugin — task-claimed hook.
# Posts a message when a new task lands in a worker's inbox.
#
# Invoked by @nps-kit/agents with env vars: NPS_TASK_ID, NPS_AGENT_ID,
# NPS_STATUS, NPS_COST_NPT, NPS_EVENT.

set -euo pipefail

PLUGIN_DIR="$(cd "$(dirname "$0")/../../plugins/discord" 2>/dev/null && pwd || true)"
if [[ -z "$PLUGIN_DIR" || ! -f "$PLUGIN_DIR/config.json" ]]; then
    # Plugin not configured — silently do nothing.
    exit 0
fi
...
# Empty config = safe fallback, no posts.
...
TEMPLATE=$(python3 -c "
import json
print(json.load(open('$CONFIG')).get('messages', {}).get('task_claimed', '\U0001f528 {account} claimed {task_id}'))
")
